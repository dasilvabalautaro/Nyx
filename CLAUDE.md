...
>
> **Synced with Krypta on 24 Sep 2026 — 79 upstream commits triaged, see
> [docs/SYNC-KRYPTA.md](docs/SYNC-KRYPTA.md).** What landed changes how the app stores things,
> so read this before touching `:data` or the identity. The **Ed25519 identity is wrapped with
> an Android Keystore key** (`IdentityStore`, pref `ed25519_wrapped`; the plaintext copy migrates
> itself on first launch). **`NyxDatabase` is v6**: `contacts.sharedSecret` is gone
> (`MIGRATION_5_6`, our own — Krypta's v6 has no `likes`/`blocked_peers`) and the secret is
> derived on read by `RoomContactRepository` via ECDH, because the stored value alone decrypted
> the whole history. The **database is SQLCipher-encrypted**, passphrase wrapped by the Keystore
> in `nyx_db` prefs; always obtain the factory through `SqlCipher.openHelperFactory` — loading
...
> (`Y` envelope, which here may only wrap content — never an `L`), copy + tappable links, and a
> batch of delivery/node hardening. **Deployed to both boxes on 24 Sep 2026** (`check-nodes.sh`:
> both "buzón · wake · relay · vuelta · ciego ok", same PeerIDs) and **verified on the TECNO's
> real data** (same PeerID, history intact, second launch OK; PRUEBAS-PENDIENTES §17). Gotcha
> found testing: HiOS **freezes the `:data` instrumented-test process** mid-suite
> (`do_freezer_trap`); `adb shell dumpsys meminfo <pid>` thaws it.
>
...
regenerated here (the `.aar` is not in git); the node half went live on **both** boxes with the
24 Sep 2026 redeploy (see the 24 Sep sync note above). Upstream verified the same code against three live nodes and on device; here it is covered
